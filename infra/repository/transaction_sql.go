...
const (
	sqlCreatePixTransaction       = "INSERT INTO pix_transactions (id, order_id, payment_id, amount, status, qr_code, payment_gateway, qr_code_link, type) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)"
	sqlFindTransactionByOrderId   = "SELECT id, order_id, payment_id, amount, status, qr_code, payment_gateway, qr_code_link FROM pix_transactions WHERE order_id = $1 LIMIT 1"
...
func (repository *DefaultTransactionRepository) CreatePixTransaction(ctx context.Context, transaction *transaction.PixTransaction) error {
	return repository.Sql(sqlCreatePixTransaction).
		Values(transaction.Id, transaction.OrderId, transaction.PaymentId, transaction.Amount, transaction.Status, transaction.QrCode, transaction.PaymentGateway, transaction.QrCodeLink, pix).
		Update(ctx)
}
...
func (repository *DefaultTransactionRepository) UpdatePixTransaction(ctx context.Context, transaction *transaction.PixTransaction) error {
	return repository.Sql(sqlUpdatePixTransaction).
		Values(transaction.Status, transaction.Id).
		Update(ctx)
}
...
func (repository *DefaultTransactionRepository) FindByOrderId(ctx context.Context, id string) (*transaction.PixTransaction, error) {
	...
	found, err := repository.Sql(sqlFindTransactionByOrderId).
		Values(id).
		FindOne(ctx, &pixTransaction.Id, &pixTransaction.OrderId, &pixTransaction.PaymentId, &pixTransaction.Amount, &pixTransaction.Status, &pixTransaction.QrCode, &pixTransaction.PaymentGateway, &pixTransaction.QrCodeLink)
	if err != nil || !found {
		return nil, err
	}
